...
for(t in 2:(n.years*12)){
  ...
  if(t %% 12 == 2){  # births happen in June, model starts in May
    ...
    fawns_born <- sum(rbinom(1, (St.f[1, t-1] + I_fawn), fawn.preg.draw)*fawns.fawn) + sum(rbinom(1, (St.f[2, t-1] + I_juv), juv.preg.draw)*fawns.juv) + rbinom(1, (sum(St.f[3:n.age.cats, t-1]) + sum(I_adults)), ad.preg.draw)*fawns.ad 
