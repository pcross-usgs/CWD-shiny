...
This shiny application is a predator-prey-disease model to explore different scenarios associated with chronic wasting disease (CWD). This tool allows the user to enter parameters for deer or elk vital rates, hunting mortality, and disease transmission and the model will plot the total number of individuals, prevalence, age and sex distribution, and how many deaths were natural, hunting or CWD-related. The user can tailor the model to their species and region of interest by modifying the parameters and starting conditions.   

This application introduces natural predators into models that respond to prey abundance in structuring predation (functional response interaction of prey population sizes and selection) and predator populations (numeric response based on prey population size).

### Model tabs

There are two other pages in this application that can be accessed via the tabs above. The deterministic model, as the name implies, includes no stochasticity. Parameters can be entered and changed using the sliders on the left hand side of the screen and any time a parameter is changed the model will automatically re-run and plot the new results. There is also a comparison tab that allow for a side-by-side comparison of two different scenarios. Note that the default parameters are initially identical between the two scenarios and need to be changed for the comparison plots to be useful.  
...
| |  | |  | |
|Predation     | number of predators | $N_{predators}$ | n.predators = 70 | number of initial predators |
|              | minimum predators | min $N_{predators}$ | min.predators = 45 | minimum number of predators for functional and numeric responses |
|              | maximum predators | max $N_{predators}$ | max.predators = 150 | maximum number of predators for functional and numeric responses |
|              | maximum kill rate | $K$ | k_max = 1 | maximum kill rate |
|              | numeric response |  | numeric.form = "Type2" | numeric response type either either Michaelis-Menten Type II or Type III functional response (Bolker, 2008) |
|              | numeric inflection point | $n_{inflection}$ | n_inflect = 2000 | numeric inflection point |
|              | functional response |  | numeric.form = "Type2" | functional response type either either Michaelis-Menten Type II or Type III functional response (Bolker, 2008) |
|              | functional inflection point | $k_{inflection}$ | k_inflect = 3000 | functional inflection point |
|              | selection form | | selection form either exponential or linear | selection scales linearly or exponentialy with infection progression |
|              | selection growth | $r$ | r = 0.35 | r in either linear or exponential growth rate |
|              | baseline selection |  | base.juv = 3, base.adult = 1, base.old = 2 | base selection for specific age classes |
|              | stages | $I_{1}-I_{10}$ | 1:10 | which disease progression classes to include in calculate in selection |
| |  | |  | |
...
|                | transmission coefficient multiplier | $S$ | S = 5 | late-stage infectivity multiplier |
...
The original CWD model was developed by the U.S. Geological Survey, Northern Rocky Mountain Science Center, in collaboration with Montana Fish, Wildlife, and Parks. The tool may be cited as:
...
Version >= 0.2.0 
Cross, PC, W Rogers, and E Brandell. 2022. Interactive chronic wasting disease model for scenario planning. Version 0.2.0 https://doi.org/10.5066/P93XICBO

which also relates to USGS IPDS #IP-137921 and the following publication: 
