#' Plot the predation deaths by stage of infection.
#'
...
#'
#' @return a plot of predation deaths by infectious category
#' 
#' @import ggplot2
#' @import dplyr
#' @importFrom magrittr %>%
...
plot_predation_stage_prop <- function(dat.prey, dat.pred, dat.params){
  if(missing(dat.pred) |
     missing(dat.params) |
     missing(dat.prey)) warning("missing data to plot")
  
  prey.pop <- dat.prey %>% 
    mutate(class = case_when(str_detect(category,"S") ~ 0,
                             str_detect(category,"I1t") ~ 1,
                             str_detect(category,"I2") ~ 2,
                             str_detect(category,"I3") ~ 3,
                             str_detect(category,"I4") ~ 4,
                             str_detect(category,"I5") ~ 5,
                             str_detect(category,"I6") ~ 6,
                             str_detect(category,"I7") ~ 7,
                             str_detect(category,"I8") ~ 8,
  ...
  pred.prop <- dat.pred %>% 
    ...
    mutate(class = case_when(str_detect(category,"H") ~ 0,
                             str_detect(category,"I1f") ~ 1,
                             str_detect(category,"I1m") ~ 1,
                             str_detect(category,"I2") ~ 2,
                             str_detect(category,"I3") ~ 3,
                             str_detect(category,"I4") ~ 4,
                             str_detect(category,"I5") ~ 5,
  ...
  p <- ggplot() +
    geom_line(data = pred.prop, mapping = aes(x = year, y = prop, color = factor(class))) +
    theme_light(base_size = 18) +
    theme(panel.grid.minor = element_blank(),
          panel.grid.major = element_blank()) +
    labs(x = "Year",
         y = "Deaths / Population size") +
    scale_color_viridis_d("Disease Stage", labels = c("Healthy",1:10)) +
    theme_light(base_size = 18) +
    theme(panel.grid.minor = element_blank(),
          panel.grid.major = element_blank()) 
